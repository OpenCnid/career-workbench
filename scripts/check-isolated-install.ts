...
try {
  ...
  await writeFile(
    ...
    [
      ...
      'const domain = await import("@career-workbench/domain");',
      'const storage = await import("@career-workbench/storage");',
      'const application = await import("@career-workbench/application");',
      'const importer = await import("@career-workbench/career-ops-import");',
      'const server = await import("@career-workbench/server");',
      ...
      'if (domain.DOMAIN_CONTRACT_VERSION !== "v1") throw new Error("domain runtime missing");',
      'if (storage.STORAGE_SCHEMA_VERSION !== 6) throw new Error("storage runtime missing");',
      'if (application.APPLICATION_VERSION !== "0.1.0-preview.0") throw new Error("application runtime missing");',
      'if (importer.CAREER_OPS_REVISION !== "3a067ee580b7982cf5dd6edf7895112e4e99600b") throw new Error("importer revision mismatch");',
      'if (typeof server.createServer !== "function") throw new Error("server runtime missing");',
      'await access(fileURLToPath(import.meta.resolve("@career-workbench/web/dist/index.html")));',
